...
- There is no test runner, linter or formatter installed. Do not assume `npm test` exists.
...
- Native iOS builds only run in GitHub Actions: `.github/workflows/build-ios-dev-client.yml` (manual `workflow_dispatch`, macOS, Expo prebuild, unsigned IPA artifact; no EAS, no Apple credentials). `app/ios` is generated by prebuild and git-ignored. The free plan's Actions minutes are limited and were exhausted in September 2026 (see `STATUS.md`): check usage and get the user's OK before dispatching any run.
...
- Ask before anything hard to undo: force-push or history rewrite, changing repo visibility, dispatching Actions runs, deleting runs.
