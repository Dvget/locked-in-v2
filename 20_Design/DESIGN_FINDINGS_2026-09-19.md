...
## Canvas 2: "LOCKED IN · Design-Richtungen" (2026-09-24)

Link (private, claude.ai): https://claude.ai/artifact/WeGiAQGrZVE4vGH52aJi5T

Row 1 shows the current app (Dashboard, active workout) as reference. Row 2 shows one proposal derived from the GymMane reference (`DESIGN_REFERENCES.md`, Inspiration 3): near-black background with a faint dot grid, one accent color, white primary buttons, hierarchy through type instead of color. Screens: Dashboard (hero start card, week row Mo to So, four compact tiles with mini trends, floating tab bar with a start button), active workout (segmented exercise progress, "LETZTES MAL / NÄCHSTES" lines, rest block with tick bar, all sets as a table with check circles), onboarding step 2 of 5 (weekly goal, D-061). The accent color is a tweak with four options (brand orange, copper, sage, off-white). Not decided; for the design round after the functional review.
