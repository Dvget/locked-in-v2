...
| D-050 | provisional | Design direction: keep the legacy look (colors, cards, spacing, footer, hierarchy) as the base and improve it conservatively. The legacy design tokens in `20_Design/DESIGN_FINDINGS_2026-09-19.md` are canonical (brand orange `#FC4C02`, running purple `#8C57F5`, card `#131316`, radius 20). Until the user picks a direction, the Dashboard follows the "Original improved" proposal from that file. |
| D-051 | provisional | Autonomous rebuild default: persistence is `expo-sqlite` on the phone behind a `Repository` interface (`app/src/data`). The web preview uses an in-memory repository with demo data (nothing is persisted there). Small preferences and unfinished sessions live in a key-value table. |
| D-052 | provisional | **Revised by the user on 2026-09-19:** the Dashboard keeps the legacy structure. One start card ("Workout starten", or "Workout/Run fortsetzen" for an unfinished session) leads to the choice screen "Workout oder Run?". The cards Workouts, Runs, Steps and Gewicht each open their own detail screen with a graph of only the last 4 weeks plus a week-to-date comparison. The full history stays in Progress (D-013). The Dashboard has no other tiles (no second start card, no report or achievement card); the Weekly Report is reachable from Progress and opens once per new week. |
| D-053 | provisional | Weekly Report compares the latest completed week with the week before, shows only metrics that exist in both weeks, and opens once automatically at the first app start of a new week (key = Monday date) as well as from the Dashboard card. Weight tone follows the goal direction (D-016). |
| D-054 | provisional | Steps come from the iPhone pedometer (`expo-sensors`, CoreMotion), last 7 days, one value per day (replaced per day), synced at app start and on foreground at most every 30 minutes when enabled in Goals. Only completed days count in averages. HealthKit is not used. |
| D-055 | provisional | Weight is entered manually. Default goal direction is "maintain". Goals in Settings: weight direction, workouts per week, runs per week, weekly step goal, global rep range (D-018). |
| D-056 | provisional | Charts are drawn with `react-native-svg` (custom `LineChart`/`BarChart`); no chart library. |
| D-057 | provisional | Running: the legacy filter, distance, rolling pace and km-split logic are ported as pure TypeScript (algorithm version `lockedIn-rn-gps-v3`, same thresholds as legacy v1/v2). Elevation is not computed and shown as "–" (Issue #19). Raw GPS points are stored per run with accepted/rejected flag (D-044). Background recording uses an `expo-location` task; spoken km cues use `expo-speech`. Live Activity and lock-screen controls are not built. |
| D-058 | provisional | Backup JSON keeps the legacy `BackupPayload` shape (dates as seconds since 1970) plus `schemaVersion: 2`, `runTrackPoints` and `lastCompletedPlanID`. Legacy files without schema version stay importable (route points are derived from `nativeArchive.route`). Polar-era run fields are dropped. Before an import replaces the data, a recovery snapshot is stored in the app database. Optional automatic backup into a user-chosen folder (directory picker; unverified on device). |
| D-059 | provisional | Workout rest timer: wall-clock based (survives backgrounding), finished cue = haptics + short spoken "Pause vorbei" (switchable) + a local notification while the app is in the background. Live Activity is not built. Exercises can be skipped, revisited and switched to stored alternatives during a workout (D-021, D-022). |
| D-061 | fixed | The app needs an onboarding process (requested by the user on 2026-09-20). Requirement is fixed; content, length and design are open and to be defined with the user. Candidate topics, not decided: goals (weight direction, workouts and runs per week), body weight, first training plan or import of a legacy backup, and asking for permissions (location, motion, notifications) in context with a short explanation instead of surprising the user at first use. Must be skippable where sensible and must not block the fast-use layer afterwards. |
| D-060 | provisional | Achievements (D-027..D-029) are computed from history, never stored: a personal best is a strict improvement over an earlier value of the same subject (best estimated strength per exercise, longest run, fastest pace for runs of at least 3 km). The first value is a baseline, not a record. The full history is listed under Progress > Bestwerte. |
