...
- **Requirement added 2026-09-20 (D-061):** the app needs an onboarding process. Open: content and design. Details in `10_Product/SCREENS_AND_FLOWS.md`. Plan to define it in the screen-by-screen session, then build it after the functional sign-off and before or together with the design track.
- **Design inspiration (updated 2026-09-24):** the user finds the current look generic ("AI slop"). Mobbin MCP needs a paid plan (about 10 to 15 USD/month); free route chosen for now: the user sends screenshots, they are described as text. References so far in `20_Design/DESIGN_REFERENCES.md`: (1) Strava chart, (2) "GOD MODE" GitHub-card thumbnail (one accent, big type, cards by tone), (3) **GymMane** (open-source offline gym app, GPL-3.0: ideas only, never copy code or art): near-monochrome, one muted accent, white primary button, hierarchy by type, set table in the live session, onboarding with a "why" line per step. Local images: `20_Design/References/` (git-ignored).
- **Design canvas 2** (Claude Design, private): https://claude.ai/artifact/WeGiAQGrZVE4vGH52aJi5T. Row 1 = current app (reference), row 2 = proposal "near black and white, one accent, white primary button" with Dashboard, active workout (set table) and onboarding step 2; accent color is a tweak (orange, copper, sage, off-white). Waiting for the user's feedback. Not decided.
- **Lessons from this chat (apply in every session):** read the legacy SwiftUI view code (and the legacy UI-preview renders in the GitHub Actions artifacts of `Dvget/locked-in`) before building a screen; never guess layout or invent tiles; ask before changing navigation logic. Shell commands with backticks or `${...}` inside `node -e` or `sed` break: use the Edit/Write tools for template strings.
